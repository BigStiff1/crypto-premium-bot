...
# In-memory storage (lost on restart — perfect for Render free tier)
user_alerts = {}          # {user_id: [alert_dicts]}
user_counts = {}          # {user_id: count_today}
user_premium = {}         # {user_id: True}
user_last_reset = {}      # {user_id: date} for daily free tier reset
total_alerts_today = 0    # Global counter for /stats social proof
...
💰 *Earn passive income?* No — *you* pay once for lifetime access.
...
async def trending(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("❌ Could not fetch trending coins right now.")
...
• Alerts set today: {total_alerts_today}
• Premium users: {len([u for u in user_premium.values() if u])}
• Uptime: Always on (Render 24/7)
...
async def set_alert(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_premium and user_counts.get(user_id, 0) >= 3:
        await update.message.reply_text("⚠️ Free limit reached (3/day).\nSend /upgrade for lifetime unlimited!")
        return
    ...
    try:
        ...
        r.raise_for_status()
        coin_list = r.json()
        coin_data = next((c for c in coin_list if c["symbol"].upper() == symbol or c["id"].lower() == symbol.lower()), None)
...
async def myalerts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not alerts:
        await update.message.reply_text("No active alerts yet.\nSet some with /alert!")
        return
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data in ["show_upgrade", "show_trending", "quick_alert", "show_myalerts"]:
        # Handle quick buttons from /start
        if data == "show_upgrade":
            await upgrade(update, context)
        elif data == "show_trending":
            await trending(update, context)
        elif data == "show_myalerts":
            await myalerts(update, context)
        elif data == "quick_alert":
            await update.message.reply_text("Send /alert BTC 65000 above to create one!")
        return
...
async def successful_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_premium[user_id] = True
    extra = "Priority checks enabled!" if "pro" in payload else "VIP perks unlocked!"
    ...
    try:
        ...
        for uid, alerts in list(user_alerts.items()):
            for alert in alerts[:]:
                ...
                triggered = (alert["condition"] == "above" and current >= alert["target"]) or \
...
def main():
    ...
    # Render webhook (perfect for Render.com web service)
